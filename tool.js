...
function processFile() {
  ...
  const filteredData = filterByTransactiondatetime(excelData);
  ...
  alert(`${processedData.length} T+1 rows processed successfully`);
}
...
async function processapi() {
  ...
  alert(`${processedData.length} T+0 rows processed successfully`);
}
...
function downloadCSV() {
  ...
  let fileName;
  if (processType === "t0") {
    fileName = "processed_settlement_report_t0.csv";
  } else {
...
async function convertToExcel() {
  ...
  try {
    ...
    if (files.length === 0) {
      status.innerHTML = "No TXT files selected.";
    ...
    for (const file of files) {
      try {
        status.innerHTML =
          `Processing ${processedFiles + 1} of ${files.length}<br>` +
          `${file.name}`;
...
function parseFixedWidth(line) {
  // Make sure the line is exactly 145 characters
  ...
  // 118-126 is the settlement date
  // It is present in the TXT but is not one
  // of the requested Excel columns.
