...
function selectTime() {
  ...
  const fromInput =
    document.getElementById("fromDateTime");

  const toInput =
    document.getElementById("toDateTime");
...
async function loadT0Businesses() {
  ...
  try {
    ...
    result.data.forEach((item) => {
      ...
      const foreeId =
        String(item.business_id).slice(-4);
...
function readTXT(event) {
  ...
  if (
    ...
    alert(
      "Please process the CSV file before uploading TXT files."
    );
...
function parseFixedWidth(line) {
  ...
  const foreeId =
    line.substring(13, 27).trim();


  const mapReferenceNumber =
    line.substring(33, 70).trim();


  const payerId =
    line.substring(70, 82).trim();


  const payerBranch =
    line.substring(82, 90).trim();


  const amount =
    Number(
      line.substring(90, 104)
    ) / 100;


  const date =
    line.substring(104, 112).trim();


  const time =
    line.substring(112, 118).trim();


  const paymentChannel =
    line.substring(126, 127).trim();
  ...
  const stan =
    line.substring(133, 139).trim();


  const authId =
    line.substring(139, 145).trim();
...
function downloadProcessedCSV() {
  ...
  if (
    ...
    alert(
      "Please process the CSV file first."
    );
  ...
  if (processType === "t0") {
    ...
    link.download =
      "processed_settlement_report_t1.csv";
  }
